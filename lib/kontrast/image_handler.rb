...
module Kontrast
    class ImageHandler
        ...
        def crop_images(test)
            # Load images
            test_image = Image.read("#{@path}/#{test.width}_#{test.name}/test.png").first
            production_image = Image.read("#{@path}/#{test.width}_#{test.name}/production.png").first
        ...
        def create_thumbnails(test)
            ...
            test_image = Image.read("#{@path}/#{test.width}_#{test.name}/test.png").first
            production_image = Image.read("#{@path}/#{test.width}_#{test.name}/production.png").first
            diff_image = Image.read("#{@path}/#{test.width}_#{test.name}/diff.png").first

            # Crop images
            Workers.map([test_image, production_image, diff_image]) do |image|
                filename = image.filename.split('/').last.split('.').first + "_thumb"
                image.resize_to_fill(200, 200, NorthGravity).write("#{@path}/#{test.width}_#{test.name}/#{filename}.png")
            end
        end

        # We upload the images per test
        def upload_images(test)
            Workers.map(Dir.entries("#{@path}/#{test.width}_#{test.name}")) do |file|
                next if ['.', '..'].include?(file)
                Kontrast.fog.directories.get(Kontrast.configuration.aws_bucket).files.create(
                    key: "#{Kontrast.configuration.remote_path}/#{test.width}_#{test.name}/#{file}",
                    body: File.open("#{@path}/#{test.width}_#{test.name}/#{file}"),
                    public: true
                )
            end
        end
